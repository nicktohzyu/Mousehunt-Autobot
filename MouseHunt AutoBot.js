...
// // Reload MouseHunt page for changes to take effect.
{
	...
	var NoCheeseSound = 'https://raw.githubusercontent.com/nicktohzyu/MH/master/sound.mp3'; //barber violin concerto mvt3 hilary hahn
	var NoCheeseSoundVolume = 0.1;
	...
	var krStartHourDelayMin = 1;
	var krStartHourDelayMax = 5;
	...
	// // Max time to wait after trap selector clicked (in second)
	var secWait = 7;

	// // Time to wait after bait changed then refresh page (in second)
	var reloadSecAfterChangeBait = 3;
	...
	var objBestTrap = {
		...
		base: {
			luck: ['Minotaur Base', 'Clockwork Base', 'Rift Base', 'Attuned Enerchi Induction Base', 'Dog Jade Base', 'Monkey Jade Base', 'Sheep Jade Base', 'Depth Charge Base', 'Horse Jade Base', 'Snake Jade Base', 'Dragon Jade Base', 'Eerier Base', 'Papyrus Base'],
...
// WARNING - Do not modify the code below unless you know how to read and write the script.
{
	...
	var huntLocation;
	var currentLocation;
	var checkTime; //timer until page refresh in sec
	var hornRetryMax = 10;
	var hornRetry = 0;
...
function sendGiftsToFavourites() {
	...
	var listOfTeammates = null;
	document.getElementsByClassName("free_gifts")[0].children[0].click(); //click to open gifts tab
	setTimeout(selectTab, rand(1000, 1100));//wait for it to load

	function selectTab() {
		document.getElementsByClassName("giftSelectorView-tabHeader")[1].click()
		setTimeout(selectGift, rand(200, 250));
	}
	function selectGift() {
		document.getElementsByClassName("giftSelectorView-gift sendable gift gift_of_the_day")[1].click()
		setTimeout(loadFriends, rand(200, 250));
	}
	function loadFriends() {
		listOfFavourites = document.getElementsByClassName("giftSelectorView-friend favorite");
		listOfTeammates = document.getElementsByClassName("giftSelectorView-friend-group team")[0].getElementsByClassName("giftSelectorView-friend");
		//console.log(listOfTeammates);
		if (listOfFavourites.length < 1) {
			console.log("error, unable to get list");
			return;
		} else {
			setTimeout(selectFavourites, rand(200, 250), 0);
		}
	}

	function selectFavourites(n) {
		//console.log("select favourites: " + n);
		while (n < listOfFavourites.length && listOfFavourites[n].className != "giftSelectorView-friend favorite") {
			n++;
		}
		if (n < listOfFavourites.length) {
			//console.log("selecting favourites " + n);
			listOfFavourites[n].click();
			setTimeout(selectFavourites, rand(150, 200), n + 1);
		} else {
			setTimeout(selectTeammates, rand(150, 200), 0);
		}
	}

	function selectTeammates(n) {
		//console.log("select teammate: " + n);
		while (n < listOfTeammates.length && listOfTeammates[n].className != "giftSelectorView-friend") {
			n++;
		}
		if (n < listOfTeammates.length) {
			//console.log("selecting teammate " + n);
			listOfTeammates[n].click();
			setTimeout(selectTeammates, rand(150, 200), n + 1);
		}
	}
}

function returnRaffle() {
	console.log("returning raffle tickets");
	document.getElementsByClassName("menuItem first inbox")[0].click();//open inbox tab
...
function locationBotCheck(caller) {
	//if (debug) console.log("RUN locationBotCheck(" + caller + ")");
	...
	switch (selAlgo) {
		case 'Hunt For':
			huntFor();
			break;
		case 'Charge Egg EggstraCharge':
			checkCharge(18, true);
			//setTimeout(easter2019ECC, 2000); //bodge
			break;
		case 'Charge Egg':
			checkCharge(18, false);
			//setTimeout(easter2019noECC, 2000); //bodge
			break;
		...
		case 'GWH2016R':
			...
			break;
		case 'All LG Area':
			{ //put this shit inside the function
				var objLGTemplate = {
					isAutoFill: false,
					isAutoPour: false,
					maxSaltCharged: 25,
				...
				var temp = getStorageToObject("LGArea", objDefaultLG);
				lgGeneral(temp);
			}
			break;
		case 'Queso Canyon':
...
function rand(min, max) {
	if (max < min) {
		console.log("rand call error, max<min", new Error().stack);
	}
	return Math.floor((Math.random() * (max - min)) + min);
}

function eventBotCheck(caller) {
	//if (debug) console.log("RUN eventBotCheck(" + caller + ")");
	if (eventBot != null && eventBot != "") {
...
function beanstalk() {
	...
	function smartArmLeapingCheese() {
		if (locationData.castle.is_boss_chase) {
			armPlainCheese();
			return;
		}
		if (locationData.castle.room_position == 19) {
			armPlainCheese();
			return;
		}
		armLeapingCheese();
	...
	function isCommonRoom() {
		return isLapisRoom() || isRubyRoom();
	}
	function isHarpRoom() {
...
function valourRift() {
	...
	//console.log("can claim?" + locationData.can_claim);
	if (locationData.can_claim) {
		console.log("claiming loot");
		document.getElementsByClassName("valourRiftHUD-claimButton mousehuntActionButton")[0].click();
	}

	function armDisarmFire(arm) {
		if (arm && !user.quests.QuestRiftValour.is_fuel_enabled) {
			var fuel = parseInt(document.getElementsByClassName("valourRiftHUD-fuelContainer-quantity quantity")[0].innerText);
			console.log("fuel remaining: " + fuel);
			if (fuel > 0) {
				document.getElementsByClassName("valourRiftHUD-fuelContainer-armButton")[0].click();
			} else {
				playAlertSound();
			}
		} else if (!arm && user.quests.QuestRiftValour.is_fuel_enabled) {
			document.getElementsByClassName("valourRiftHUD-fuelContainer-armButton")[0].click();
		}
	}
}

//plan: bot decides to travel, setStorage targetLocation, then goes to https://www.mousehuntgame.com/travel.php, bot identifies webpage and calls travelToLocation
function travelToLocation() { //early draft. should only run on https://www.mousehuntgame.com/travel.php
	targetLocation = getStorage(targetLocation);
	if (targetLocation == null) {
		return;
	}
	//code
	if (targetLocation === getCurrentLocation()) {
		setStorage("targetLocation", null);
	}
}
...
function bwRift() { //interface does not work; settings must be done in code
	...
	if (!(objUser.status_effects.un.indexOf('default') > -1 || objUser.status_effects.un.indexOf('remove') > -1) ||
		...
		nIndexBuffCurse = 8;  //cursed
	else {
		if (objUser.status_effects.ng.indexOf('default') < 0) //paladin's bane?
			nIndexBuffCurse |= 0x04;
		if (objUser.status_effects.ac.indexOf('default') < 0) //acolyte influence?
	...
	if (nIndex === 0 || objUser.chamber_status == 'open') {
		...
		if (storePortalHistory) {
			...
			var portalHistoryCurrentIndex = portalHistory.length;
			portalHistory[portalHistoryCurrentIndex] = [];
			portalHistory[portalHistoryCurrentIndex][0] = "Date: " + Date();
			portalHistory[portalHistoryCurrentIndex][1] = "nIndex: " + nIndex + "  Chamber status: " + objUser.chamber_status;
		}
		...
		if (classPortalContainer.length > 0) {
			...
			for (i = 0; i < objPortal.arrName.length; i++) {
				...
				if (classPortalContainer[0].children[i] == 'frozen') {
					console.log("Portal", i, "is frozen");
			...
			if (bwRiftConfig.choosePortal) {
				if (nIndex === 0 || (nIndex > 0 && objUser.chamber_status == 'open' && bwRiftConfig.choosePortalAfterCC)) {
					...
					if (objPortal.arrIndex[nMinIndex] == Number.MAX_SAFE_INTEGER){
						nIndex = nIndexOld;
						selectPortal = false;
					}*/
					console.log("Choose portal: ", objPortal);
					if (storePortalHistory) {
...
function pricklyPlains() {
	...
	//get stored data
	var storageValue = window.localStorage.getItem('PricklyPlains');
	//console.log("in pricklyPlains(), storageValue:", objPricklyPlains);
	var objPricklyPlains;
	if (isNullOrUndefined(storageValue)) {
		var defaultObjPricklyPlains = {
			bait: 'Bland Queso'
		};
		//storageValue = JSON.stringify(defaultObjPricklyPlains);
		//window.localStorage.
		objPricklyPlains = defaultObjPricklyPlains;
	} else {
		objPricklyPlains = JSON.parse(storageValue);
	}

	//equip traps
	checkThenArm(null, "base", "Attuned Enerchi Induction Base");
	checkThenArm(null, "weapon", "Event Horizon Trap");
...
function quesoGeyser() { // create object with equipment to use
	...
	var t = document.getElementsByClassName("quesoGeyserHUD-stateBlock mousehuntTooltipParent")[0];
	var state = t.getElementsByClassName("quesoGeyserHUD-block-title quesoGeyserHUD-stateName")[0].innerText; // type of cork or eruption or null
	console.log("Current queso geyser state:", state);

	var craftingItemElement = document.getElementsByClassName("quesoGeyserHUD-craftingItem-quantity quantity");
	var craftingItemQuantity = [];
	for (i = 0; i < 4; i++) {
		craftingItemQuantity[i] = parseInt(craftingItemElement[i].innerText);
	}
	//console.log("Crafting Item Quantities:", craftingItemQuantity);
	var quesoQuantityElement = document.getElementsByClassName("quesoHUD-bait-group-baitQuantity quantity");
	var quesoQuantity = [];
	for (i = 0; i < 6; i++) {
		quesoQuantity[i] = parseInt(quesoQuantityElement[i].innerText);
	}
	var quesoArmedElement = document.getElementsByClassName("quesoGeyserHUD-baitContainer")[0].children;
	var quesoArmed = -1; //which queso is armed
	for (i = 0; i < 6; i++) {
		if (quesoArmedElement[i].className.indexOf("active") >= 0) {
			quesoArmed = i;
		}
	...
	//console.log("Queso Quantities:", quesoQuantity);
	//console.log("Queso Armed:", quesoArmed);
	var phase = "none";
	var size = 0; //small = 1, medium = 2, large = 3, epic = 4
	if (state != "null") {
		if (state.indexOf("Cork") >= 0) {
			phase = "corked"
		}
		else if (state.indexOf("Eruption") >= 0) {
			phase = "eruption"
		}
		console.log("Phase:", phase);
		var huntsRemaining = parseInt(document.getElementsByClassName("quesoGeyserHUD-block-huntsRemaining")[0].innerText); // number of hunts remaining
		console.log("Hunts remaining:", huntsRemaining);
		if (state.indexOf("Small") >= 0) {
			size = 1
		}
		else if (state.indexOf("Medium") >= 0) {
			size = 2
		}
		else if (state.indexOf("Large") >= 0) {
			size = 3
		}
		else if (state.indexOf("Epic") >= 0) {
			size = 4
		}
		console.log("Size:", size);
	}
	if (phase == "corked") {
		var currentPressure = parseInt(document.getElementsByClassName("quesoGeyserHUD-eruptionPressure")[0].innerText); //does not dissapear or reset during eruption
		var pressureArr = [0, 35, 175, 1000, 3000];
		var maxPressure = pressureArr[size];
		var pressureNeeded = maxPressure - currentPressure;
		var presurePerHuntNeeded = pressureNeeded / huntsRemaining;
		var maxCatchPressure = [-1, 5, 15, 40, 100, 1500]; //max increase possible per cheese type
		console.log("Current pressure:", currentPressure, "Max:", maxPressure, "Pressure needed:", pressureNeeded, "Pressure per hunt needed:", presurePerHuntNeeded.toFixed(2));
		//use lower cheese if pressure almost full
		/*if(pressureNeeded <= maxCatchPressure[quesoArmed-1]){ //not in use
			quesoArmedElement[quesoArmed-1].getElementsByClassName("mousehuntItem-boundingBox")[1].click(); //click on the previous cheese then refresh page
		...
		if (huntsRemaining > 0) {
			...
			armTonic(eruptionTonicList[size + 1]);
		}
		...
		} else if (craftingItemQuantity[1] >= 15) {
			if (craftingItemQuantity[0] >= 30) {
				corkSize = 2;
			}
		} else if (craftingItemQuantity[0] >= 10) {
			if (craftingItemQuantity[0] >= 10) {
				corkSize = 1;
			}
		} else {
			corkSize = 0;
		}
		console.log("cork size to build:", corkSize);
		if (corkSize > 0 && buildCork) {
			document.getElementsByClassName("quesoHUD-confirm-state buildCork")[0].getElementsByClassName("mousehuntActionButton small")[corkSize - 1].click(); //because we defined 0 as no cork
			setTimeout(quesoGeyser, 2000);//call bot again after building cork
		}
	}
}
...
function easterMopi(){
	...
	checkThenArm(null, "weapon", "Storm Wrought Ballista Trap");
	checkThenArm(null, "bait", "Dragonvine Cheese");
}
...
function easterGrift(){ //Farm map clue egg
	...
	checkThenArm(null, "weapon", "Timesplit Dissonance Trap");
	//checkThenArm(null, "bait", "Riftiago Cheese");
...
function easterFrift(){
	...
	checkThenArm(null, "base", "Clockwork Base");
	checkThenArm(null, "weapon", "Timesplit Dissonance Trap");
	checkThenArm(null, "bait", "Rift Rumble");
}
...
function easterQRiver(){
	...
	checkThenArm(null, "bait", "Marshmallow Monterey");
}

function easter2019Travel(locationNumber){ //numbers have changed since 2019 easter
	if(document.getElementsByClassName("springHuntHUD-popup-region-environment-header").length == 0){ //need to click on hud
		//console.log("Clicking AreaEggs button to open travel menu");
		document.getElementsByClassName("springHuntHUD-areaEggs")[0].click();
	}
	setTimeout(easter2019TravelClick, 1000, locationNumber);
	setTimeout(reloadPage, 10000);
}

function easter2019TravelClick(locationNumber){
	//console.log("Clicking travel button");
	document.getElementsByClassName("springHuntHUD-popup-region-environment-header")[locationNumber].getElementsByClassName("mousehuntActionButton small")[0].click();
}
*/
...
function BurroughRift(bCheckLoc, minMist, maxMist, nToggle) {
	...
	console.plog('Current Mist Quantity:', currentMistQuantity, 'Is Misting:', isMisting, "target: ", minMist, "-", maxMist);
...
/*function ZTower() { //obsolete
	console.log("in ztower");
	var location = getPageVariable('user.location');
...
function zugzwangTower() {
	...
	*/
	var objDefaultZT = {
		focus: 'MYSTIC=>TECHNIC',
		order: ['PAWN', 'KNIGHT', 'BISHOP', 'ROOK', 'QUEEN', 'KING', 'CHESSMASTER'],
		weapon: new Array(14).fill('BPT/OAT'),
		base: new Array(14).fill(''),
		trinket: new Array(14).fill('Ultimate Ancient Charm'),
		bait: new Array(14).fill('Gouda'),
	};
	objDefaultZT.weapon[0] = "MPP/TPP";
	objDefaultZT.weapon[7] = "MPP/TPP";
	objDefaultZT.weapon[3] = "Sphynx Wrath"; //chessmaster, rook and queen
	objDefaultZT.weapon[4] = "Sphynx Wrath";
	objDefaultZT.weapon[6] = "Sphynx Wrath";
	objDefaultZT.weapon[10] = "Sphynx Wrath";
	objDefaultZT.weapon[11] = "Sphynx Wrath";
	objDefaultZT.weapon[13] = "Sphynx Wrath";
	objDefaultZT.bait[5] = "Checkmate"; //assume trying for chessmaster or double
	objDefaultZT.bait[6] = "Checkmate";
	objDefaultZT.bait[12] = "Checkmate";
	objDefaultZT.bait[13] = "Checkmate";
	objDefaultZT.trinket[0] = "none"; // no charms for pawns, attraction for kings
	objDefaultZT.trinket[5] = "Valentine Charm";
	objDefaultZT.trinket[7] = "none";
	objDefaultZT.trinket[10] = "Rook Crumble Charm";
	objDefaultZT.trinket[12] = "Valentine Charm";
	objDefaultZT.trinket[1] = "Ancient Charm"; //weaker charm for first set of knights and bishops
	objDefaultZT.trinket[2] = "Ancient Charm";
	objDefaultZT.trinket[4] = "Festive Ultimate Luck Charm"; //queens
	objDefaultZT.trinket[11] = "Festive Ultimate Power Charm"; //power is better when low amplifier

	//window.sessionStorage.setItem('ZTower', JSON.stringify(objDefaultZT));
	//window.localStorage.setItem('ZTower', JSON.stringify(objDefaultZT));
	//var objZT = objDefaultZT;
	var objZT = getStorageToObject('ZTower', objDefaultZT);

	objZT.focus = objZT.focus.toUpperCase(); //WTF?
...
function retrieveMouseList() {
	//ztower bug here
	document.getElementsByClassName("campPage-trap-trapEffectiveness")[0].click();
	var sec = secWait;
	var intervalRML = setInterval(
		function () {
			var y = document.getElementsByClassName('campPage-trap-trapEffectiveness-mouse');
			if (y.length > 0) {
				mouseList = [];
				for (var i = 0; i < y.length; ++i) {
					mouseList.push(y[i].getAttribute('title'));
				}
				document.getElementsByClassName("campPage-trap-blueprint-closeButton")[0].click();
...
// Easter event
function checkCharge(stopDischargeAt, useEggstraCharge) {
	try {
		var charge = parseInt(document.getElementsByClassName('springHuntHUD-charge-quantity')[0].innerText);
		var isDischarge = (getStorage("discharge") == "true");
		console.plog('Current Charge:', charge, 'Discharging:', isDischarge, 'Stop Discharge At:', stopDischargeAt, "Use EggstraCharge: ", useEggstraCharge);
		...
		if (charge == 20) {
			...
			checkThenArm(null, "charm", "Eggstra Charm");
		}
		else if (charge > stopDischargeAt && charge < 20) {
			if (isDischarge) {
				checkThenArm(null, "charm", "Eggstra Charm");
			}
			else {
				if (charge >= chargeHigh && eggstraCharge.quantity > 0 && useEggstraCharge) {
					checkThenArm(null, "charm", "Eggstra Charge Charm");
				}
				else {
					checkThenArm(null, "charm", "Eggscavator Charge Charm");
				}
			}
		}
...
/*function oldCheckCharge(stopDischargeAt) {
	try {
...
function CalculateDelay(trapcheck, xcheckTime, xnextActiveTime) {
	var date = new Date();
	var hourNow = date.getHours();
	var minutesNow = date.getMinutes();
	var delay = -1; // easy to spot bug
	var tbsh = botStartHour;

	if (!trapcheck) { //horn
		delay = hornTimeDelayMin + Math.round(Math.random() * hornTimeDelayVariance) + 1; //+1 sec to be safe
	} else { //trapcheck
		delay = checkTimeDelayMin + Math.round(Math.random() * checkTimeDelayVariance) + 4; //+4 sec to be safe
	}

	if (Math.random() < smallDelayProbability) {
		delay += smallDelayMin + Math.round(Math.random() * (smallDelayVariance));
		console.plog("added small delay");
	}
	if (Math.random() < extraDelayProbability) {
		delay += extraDelayMin + Math.round(Math.random() * (extraDelayVariance));
		console.plog("added extra delay");
	}

	var delayToMorning = false;
	if (botStopHour > botStartHour) {
		if (hourNow >= botStopHour) tbsh += 24;
		if (hourNow >= botStopHour || hourNow < botStartHour) {
			console.plog("Delaying until tomorrow morning");
			delayToMorning = true;
		}
	} else if (botStartHour > botStopHour && hourNow >= botStopHour && hourNow < botStartHour) {
		console.plog("Delaying until tomorrow morning");
		delayToMorning = true;
	}

	if (!trapcheck && selAlgo != "None") { //do not sound horn at the same time as trap check
		var timeToNextHorn = xnextActiveTime + delay;
		if (Math.abs(timeToNextHorn - xcheckTime) < 15) { //going to clash
			console.log("Adding extra delay to avoid horn clash with trap check");
			delay = xcheckTime - xnextActiveTime + 15 + Math.round(Math.random() * 6); //15 sec to do event location check etc
		}
	}
	var timetoMorning = 3600 * (tbsh - hourNow) - 60 * minutesNow;
	//console.log("time to morning: ", timetoMorning);
	if (delayToMorning) {
		if (trapcheck) {
			delay += Math.ceil(timetoMorning / 3600) * 3600;
		}
		else { //horn
			delay += timetoMorning - xnextActiveTime;
		}
	}
	//console.log("Calculate delay, trapcheck = ", trapcheck, " delay = ", delay);
	return delay;
}

function returnGifts() {
	x = document.getElementsByClassName("mousehuntActionButton return tiny");
	for (var i = 0; i < x.length; i++) {
		if (x[i].className.includes("disabled")) { continue; }
		else {
			x[i].click();
		}
	}
	...
	for (var i = 0; i < codeError.length; i++) {
		if (codeError[i].innerText.toLowerCase().indexOf("incorrect claim code") > -1) {
			if (kingsRewardRetry >= kingsRewardRetryMax) {
				...
				console.log("CallKRSolver from CheckKRAnswerCorrectness");
				CallKRSolver();
			}
			return;
		}
		...
		function () {
			...
			if (armStatus != LOADING) {
				...
				if (armStatus == NOT_FOUND) {
					//clearTrapList(trap);
					if (trap == 'trinket' || trap == 'bait') {
						disarmTrap(trap);
						console.log("Failed to arm", name, ", disarming current", trap);
						closeTrapSelector(trap);
					} else {
						closeTrapSelector(trap);
					}
...
function armTrapNewUI(sort, trap, name) {
	...
	var nameElement;
	var arrName = (Array.isArray(name)) ? name.slice() : [name]; //but slice() here doesn't do anything!

	if (sort == 'best' || sort == 'any')
		name = name[0];

	if (itemEle.length > 0) {
		//console.log('Trying to arm ' + name);
		for (var i = 0; i < itemEle.length; i++) {
			...
			if (nameElement.indexOf(name) === 0) {
				...
				if (trap == "bait") {
					console.log("changed bait, reloading page in", reloadSecAfterChangeBait, "seconds");
					setTimeout(reloadPage, reloadSecAfterChangeBait * 1000);
				}
				return ARMED;
			}
		}

		console.plog(trap, name, 'not found');
		for (var i = 0; i < objTrapList[trap].length; i++) {
			if (objTrapList[trap][i].indexOf(name) === 0) {
				objTrapList[trap].splice(i, 1); //removes unfound item from traplist
				setStorage("TrapList" + capitalizeFirstLetter(trap), objTrapList[trap].join(","));
				console.log("Unable to arm", trap, name, "but found in trap list, now removed");
				break;
			}
		}
		if (sort == 'best' || sort == 'any') {
...
function retrieveDataFirst() {//needs revamp
		...
		if (scriptElementList) {
			...
			for (i = 0; i < scriptElementList.length; ++i) {
				...
				if (hornTimeStartIndex >= 0) {
					...
					if (hornTime < 15) {
						hornTime = 15; //on first load, give enough time to run bot
					}
					if (nextActiveTime <= 0 && !isKingReward) { //nani??
						locationBotCheck();
					}
...
function getKingRewardStatus() {
	console.log("RUN getKingRewardStatus");
	var strValue = getPageVariable('user.has_puzzle');
	//console.log('user.has_puzzle:', strValue);
	...
	if (headerOrHud !== null) {
		...
		if (textContentLowerCase.indexOf("king reward") > -1 || textContentLowerCase.indexOf("king's reward") > -1 || textContentLowerCase.indexOf("kings reward") > -1) {
			return true;
		} else
			return (strValue == 'true');
	} else
...
function getCharmQuantity() { //works as of 2019-3-8, if no charm armed the web page value is zero
	var CharmQuantity = parseInt(document.getElementsByClassName("campPage-trap-armedItem trinket")[0].innerText.replace(/,/g, ""));
	...
	try {
		...
		if (nextActiveTime === "" || isNaN(nextActiveTime)) {
			...
			if (nextActiveTime === 0)
				...
				hornTimeDelay = CalculateDelay(false, checkTime, nextActiveTime); //&ntzy
			}
			if (debug) console.plog('Time to horn available:', nextActiveTime, 'Delay:', hornTimeDelay);
			hornTime = nextActiveTime + hornTimeDelay;
			lastDateRecorded = undefined;
			lastDateRecorded = new Date();
		}
...
function action() {
	...
	try {
		if (isKingReward) {
			...
			//console.log("no special case in action()");
			// update location
			displayLocation(huntLocation);
			...
			if (isHornSounding === false) {
				// start timer
				window.setTimeout(function () { //why not use setInterval instead??
					countdownTimer();
		...
		if (showTimerInPage) {
			...
			if (headerElement) {
				...
				if (targetPage && aggressiveMode) {
					...
					titleElement.innerHTML = "<b><a href=\"https://greasyfork.org/en/scripts/32971-mousehunt-autobot-enhanced-revamp\" target=\"_blank\">MouseHunt AutoBot ENHANCED + REVAMP (version " + scriptVersion + ")</a> + MouseHunt AutoBot Additional thing" + (isNewUI ? " ~ Beta UI" : "") + "</b>";
				}
				...
				*/
				...
				preferenceHTMLStr += '<tr style="display:none;">';
				preferenceHTMLStr += '<td style="height:24px; text-align:right;">';
				...
				preferenceHTMLStr += '<input type="text" id="event" name="event" value="' + locationBot + '"/>';
				preferenceHTMLStr += '</td>';
				preferenceHTMLStr += '</tr>';
				*/
				...
				preferenceHTMLStr += '<tr>';
				preferenceHTMLStr += '<td style="height:24px; text-align:right;">';
				preferenceHTMLStr += '<a title="Select the script event algorithm"><b>Event Bot</b></a>&nbsp;&nbsp;:&nbsp;&nbsp;';
				preferenceHTMLStr += '</td>';
				preferenceHTMLStr += '<td style="height:24px">';
				...
				preferenceHTMLStr += '<tr>';
				preferenceHTMLStr += '<td style="height:24px; text-align:right;">';
				preferenceHTMLStr += '<a title="Select the script location algorithm"><b>Location Bot</b></a>&nbsp;&nbsp;:&nbsp;&nbsp;';
				preferenceHTMLStr += '</td>';
				preferenceHTMLStr += '<td style="height:24px">';
				...
				preferenceHTMLStr += '<option value="Charge Egg EggstraCharge">Charge Egg EggstraCharge</option>';
				preferenceHTMLStr += '<option value="Charge Egg">Charge Egg</option>';
				preferenceHTMLStr += '<option value="FG/AR">FG => AR</option>';
				preferenceHTMLStr += '<option value="Fiery Warpath">Fiery Warpath</option>';
				preferenceHTMLStr += '<option value="Fort Rox">Fort Rox</option>';
				...
				preferenceHTMLStr += '<tr id="trPricklyPlainsBait" style="display:none;">';
				preferenceHTMLStr += '<td style="height:24px; text-align:right;"><a title="Prickly Plains Bait"><b>Prickly Plains Bait</b></a>&nbsp;&nbsp;:&nbsp;&nbsp;</td>';
				preferenceHTMLStr += '<td style="height:24px">';
				preferenceHTMLStr += '<select id="selectPricklyPlainsBait" style="width: 100px;" onchange="onSelectPricklyPlainsBait();">';
				preferenceHTMLStr += '<option value="Bland Queso">Bland Queso</option>';
				preferenceHTMLStr += '<option value="Mild Queso">Mild Queso</option>';
				preferenceHTMLStr += '<option value="Medium Queso">Medium Queso</option>';
				preferenceHTMLStr += '<option value="Hot Queso">Hot Queso</option>';
				preferenceHTMLStr += '<option value="Flamin Queso">Flamin Queso</option>';
				preferenceHTMLStr += '</select>';
				preferenceHTMLStr += '</td>';
				preferenceHTMLStr += '</tr>';
...
function loadPreferenceSettingFromStorage() {
	...
	var eventTemp = getStorage('locationBot');
	if (eventTemp == undefined || eventTemp == null) {
		setStorage('locationBot', 'None');
		eventTemp = getStorage('locationBot');
	}
	locationBot = eventTemp;
	eventTemp = undefined;
	...
	*/
	...
	try {
		...
		// Disable GWH2016
		if (getStorageToVariableStr("locationBot", "None").indexOf('GWH2016') > -1)
			setStorage("locationBot", "None");
...
function noCheeseAction() {
	...
	// var checkRearmInterval = setInterval(function () { //doesn't work
	// 	try {
	// 		var ajaxBaitName = getPageText("hud_baitName");
	// 		console.log("debug noCheeseAction, current bait:", ajaxBaitName);
	...
	// 	}
	// }, 10000);
	setInterval(reloadPage, rand(18000, 25200) * 1000);//wait 5-7 hours then reload page
}
...
function kingRewardCountdownTimer(interval, isReloadToSolve) {
	...
	if (interval < 0) {
		...
		if (isReloadToSolve) {
			...
			console.log("CallKRSolver from kingRewardCountdownTimer");
			CallKRSolver();
		}
	} else {
		if (!checkResumeButton()) {
...
function bodyJS() { //don't know what this does but does not affect function
	var objDefaultFGAR = {
		order: ['FG', 'AR'],
		weapon: new Array(2).fill(''),
		base: new Array(2).fill(''),
	...
	var defaultBWRiftConfig = {
		...
		choosePortal: false,
		choosePortalAfterCC: false,
		priorities: ['SECURITY', 'FURNACE', 'PURSUER', 'GUARD', 'FROZEN', 'ACOLYTE', 'LUCKY', 'HIDDEN', 'TIMEWARP', 'RUNIC', 'ANCIENT', 'GEARWORKS', 'GEARWORKS'],
		prioritiesCursed: ['SECURITY', 'FURNACE', 'PURSUER', 'ANCIENT', 'GEARWORKS', 'RUNIC', 'GEARWORKS', 'GEARWORKS', 'GEARWORKS', 'GEARWORKS', 'GEARWORKS', 'GEARWORKS', 'GEARWORKS'],
		minTimeSand: [70, 70, 55, 55, 55, 55, 45, 45, 999],
		minRSCType: 'NUMBER',
		minRSC: 60,
		enterMinigameWCurse: false
	};
	...
	function handleFiles(files) { //WTF
		console.log("run handleFiles", files);
		if (files.length < 1)
			return;
	...
	var arrKey = ['SCCustom', 'Labyrinth', 'LGArea', 'locationBot', 'eventBot', 'FW', 'BRCustom', 'SGarden', 'Zokor', 'FRift', 'MapHunting', 'ZTower', 'BestTrap', 'Iceberg', 'WWRift', 'GES', 'FRox', 'GWH2016R', 'SpecialFeature', 'BWRift', 'BC_JOD', 'FG_AR'];
	...
	function initEventBot() {
		var algoOnLocal = window.localStorage.getItem("eventBot");
		var algoOnSession = window.sessionStorage.getItem("eventBot");
		//console.log("initeventbot: \n algoonsession: " + algoOnSession);
		try {
		...
		if (algoOnSession != algoOnLocal) {
			if (debug) console.log("initEventBot() WARNING: Session algo is different from local storage." + algoOnSession + " | " + algoOnLocal);
		}

		var eventBotSelect = document.getElementById("eventBotSelect");
		var eventBotSelectOpt = eventBotSelect.options;
		for (var opt, j = 0; opt = eventBotSelectOpt[j]; j++) {
			if (opt.value == algoOnSession) {
				eventBotSelect.selectedIndex = j;
	...
	function initLocationBot() {
		console.log("running initLocationbot");
		var algoOnLocal = window.localStorage.getItem("locationBot");
		var algoOnSession = window.sessionStorage.getItem("locationBot");
		try {
			if (algoOnSession === undefined || algoOnSession === null)
				algoOnSession = algoOnLocal;
		} catch (e) {
		...
		if (algoOnSession != algoOnLocal) {
			if (debug) console.log("initLocationBot() WARNING: Session algo is different from local storage." + algoOnSession + " | " + algoOnLocal);
		}

		var locationBotSelect = document.getElementById("locationBotSelect");
		var locationBotSelectOpt = locationBotSelect.options;
		for (var opt, j = 0; opt = locationBotSelectOpt[j]; j++) {
			if (opt.value == algoOnSession) {
				locationBotSelect.selectedIndex = j;
				showOrHideTr(algoOnSession);
				return;
			}
		}
	...
	function onInputResetReload() {
		var strValue = document.getElementById('locationBotSelect').value;
		var keyName;
	...
	function onSelectPricklyPlainsBait() {
		//console.log("run onSelectPricklyPlainsBait()");
		savePricklyPlains();
	}

	function savePricklyPlains() {
		//console.log("run saveQuesoCanyon()");
		var selectPricklyPlainsBait = document.getElementById('selectPricklyPlainsBait');
		//console.log("Selected bait:", selectPricklyPlainsBait.value);
		var storageValue = window.localStorage.getItem('PricklyPlains');
		if (isNullOrUndefined(storageValue)) {
			var objPricklyPlains = {
				bait: 'Bland Queso'
			};
			storageValue = JSON.stringify(objPricklyPlains);
		}
		storageValue = JSON.parse(storageValue);
		storageValue.bait = selectPricklyPlainsBait.value;
		window.localStorage.setItem('PricklyPlains', JSON.stringify(storageValue)); //why not local storage?
		console.log("saved data:", window.localStorage.getItem('PricklyPlains'))
	}

	function initControlsQuesoCanyon() {
		console.log("run initControlsQuesoCanyon()");
	}

	function initControlsBWRift(bAutoChangeChamber) {
		//console.log("run initControlsBWRift");
